...
function bot_run()
	...
	bot = bot.result
	local runlog = 'Umbrella API Bot, Lunched now...'
	print(runlog)
...
function send_pm(chat_id, text)
	local send = send_api.."/sendMessage?chat_id="..chat_id.."&text="..url.escape(text).."&disable_web_page_preview=true&parse_mode=HTML"
	return send_req(send)
end
...
function send_key(chat_id, text, keyboard, resize)
	local response = {}
	...
	local sended = send_api.."/sendMessage?chat_id="..chat_id.."&text="..url.escape(text).."&parse_mode=Markdown&reply_markup="..url.escape(responseString)
